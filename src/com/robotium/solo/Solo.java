...
public class Solo {
	...
	//孙建平 2015-03-10 swip 拖动
	public boolean waitForWebElement(By by,Solo solo){
		return (waiter.waitForWebElement(by, 0, Timeout.getLargeTimeout(), true,solo) != null);
	}
	
	//孙建平 2015-03-10 swip 拖动
	public boolean waitForWebElement(By by,Solo solo,int Timeout){
			return (waiter.waitForWebElement(by, 0, Timeout, true,solo) != null);
	}
...
//			Assert.fail("WebElement is null and can therefore not be clicked!");
	...
	public void drag(float fromX, float toX, float fromY, float toY, 
			int stepCount) {
		drag(fromX, toX, fromY, toY, stepCount,500);
	}
	
	public void drag(float fromX, float toX, float fromY, float toY, 
			int stepCount,int downTime) {
		dialogUtils.hideSoftKeyboard(null, false, true);
		scroller.drag(fromX, toX, fromY, toY, stepCount,downTime);
	}
	...
	public void scrollToSide(int side, float scrollPosition, int stepCount) {
		...
		case UP:  scroller.scrollToSide(Scroller.Side.UP, scrollPosition, stepCount);  break;
		case DOWN:  scroller.scrollToSide(Scroller.Side.DOWN, scrollPosition, stepCount);  break;
		}
	}
...
//			Assert.fail("WebView is not found!");
	 ...
	 * 选择下拉框
	 */
	public void selectSelected(By by, String text){
		if(waiter.waitForWebElement(by, 0, Timeout.getSmallTimeout(), false) == null) {
			throw new IllegalArgumentException("robotium error"); 
//			Assert.fail("WebElement with " + webUtils.splitNameByUpperCase(by.getClass().getSimpleName()) + ": '" + by.getValue() + "' is not found!");
		}
		webUtils.executeJavaScriptFunction("selectSelected(\"" + by.getValue()
				+ "\", \"" + text + "\");");
	...
	public void DebugJS(String js){
		webUtils.executeUserFunction(js);
	
	}
	
	public TextView getTextView(String text,int match) {
		
		return getTextView(text,match,true);
		
	}
	
	public TextView getTextView(String text,int match,Boolean scroll) {
		
		TextView textview = waiter.waitForText(text, match, Timeout.getSmallTimeout(), scroll, true, false);
		
		return textview;
		
	}
	
	public TextView getTextView(String text,int match,Boolean scroll,int outtime) {
		
		TextView textview = waiter.waitForText(text, match, outtime, scroll, true, false);
