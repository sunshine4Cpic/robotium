...
class Waiter {
	...
	//某个webview 页面 弹出 DIV 必须用SWIP滑动才能找到 不能使用pagedown
	//孙建平 2015-03-10增加
	public WebElement waitForWebElement(final By by, int minimumNumberOfMatches, int timeout, boolean scroll,Solo solo){
		final long endTime = SystemClock.uptimeMillis() + timeout;
		...
		while (true) {	
			...
			if(scroll) {
				//scroller.scrollDown();
				solo.scrollToSide(Solo.UP,0.75f,40);
				solo.sleep(100);
