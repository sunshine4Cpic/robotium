...
function xpath(xpath, click) {
	...
	var docxpath = xpath;
	var ifdoc = document;
	if(xpath.indexOf("iframe{")==0)
	{
		iframeXpath = xpath.substring(7,xpath.indexOf("},"));
		docxpath = xpath.substring(xpath.indexOf("},")+2,xpath.length);
		ifdoc = document.evaluate(iframeXpath, document, null, XPathResult.UNORDERED_NODE_ITERATOR_TYPE, null).iterateNext().contentWindow.document; 
	}
	
	var elements = document.evaluate(docxpath, ifdoc, null, XPathResult.UNORDERED_NODE_ITERATOR_TYPE, null); 
	if (elements){
...
function selectSelected(xpath, opText) {
	var element = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null ).singleNodeValue;
	if(element!=null)
	{
		for(var i=0;i<element.options.length;i++)
		{
			if(element.options[i].text == opText)
			{
...
function enterTextByXpath(xpath, text) {
	...
	var element = document.evaluate(docxpath, ifdoc, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null ).singleNodeValue;
	if(element != null)
		element.value = text;
